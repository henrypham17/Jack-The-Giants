...
public class Highscore implements Screen {   // kế thừa từ interface Screen tạo ra màn hình highscore trong game

    private GameMain game;  // một đối tượng của class GameMain, đại diện cho trò chơi
    private OrthographicCamera mainCamera; // đối tượng của class OrthographicCamera, được sử dụng để xác định toạ độ camera trong trò chơi
    private Viewport gameViewport;  // đối tượng của class StretchViewport, được sử dụng để cố định kích thước màn hình
    private Texture bg;  // đối tượng của class Texture, đại diện cho hình nền của màn hình Highscore
    private HighscoreButtons btns;  // đối tượng chứa các nút chức năng của màn hình Highscore
    ...
    public Highscore(GameMain game){
        ...
        bg = new Texture("Backgrounds/Highscore BG.png");  // hình ảnh nền của màn hình điểm số cao.

        // Khởi tạo một đối tượng HighscoreButtons với tên là btns bằng cách truyền đối tượng game vào. Đối tượng này chứa các nút (button)
        // để người chơi có thể tương tác với màn hình điểm số cao.
